...
func DecodeRequestHeader(isfb bool, first *buf.Buffer, reader io.Reader, validator *vless.Validator) ([]byte, *protocol.RequestHeader, *Addons, bool, error) {
	...
	if isfb {
		...
		if _, err := buffer.ReadFullFrom(reader, 1); err != nil {
			return nil, nil, nil, false, newError("failed to read request version").Base(err)
		}
	...
	case 0:
		...
		if isfb {
			...
			if _, err := buffer.ReadFullFrom(reader, 16); err != nil {
				return nil, nil, nil, false, newError("failed to read request user id").Base(err)
			}
			copy(id[:], buffer.Bytes())
		}

		if request.User = validator.Get(id); request.User == nil {
			return nil, nil, nil, isfb, newError("invalid request user id")
		}
		...
		if err != nil {
			return nil, nil, nil, false, newError("failed to decode request header addons").Base(err)
		}

		buffer.Clear()
		if _, err := buffer.ReadFullFrom(reader, 1); err != nil {
			return nil, nil, nil, false, newError("failed to read request command").Base(err)
		}
		...
		if request.Address == nil {
			return nil, nil, nil, false, newError("invalid request address")
		}
		return id[:], request, requestAddons, false, nil
	default:
		return nil, nil, nil, isfb, newError("invalid request version")
	}
}
